<?php
//Desert Demolition: Revised Mappings Ripper
//By Techokami
...
if(!file_exists($savepath."\\strips")) mkdir($savepath."\\strips");
...
//Let's get the level strips!
$map256 = imagecreatetruecolor(512, 256);
fseek($ram, 0x6400);
$i = 0;
for($i= 0; $i < 32; $i++)
{
	...
	for($y= 0; $y < 8; $y++)
	{
		...
		for($x= 0; $x < 16; $x++)
		{
			imagecopy($map256, imagecreatefrompng($savepath."\\".getword($ram).".png"), $x * 32, $y * 32, 0, 0, 32, 32);
		}
	}
	//Save the Map256
	imagecolortransparent($map256, $palone[0]);
	imagepng($map256, $savepath."\\strips\\".($i).".png");
}
